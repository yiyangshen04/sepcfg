...
使用者须对自身行为负全部责任,并遵守所在司法辖区的法律法规。仓库本身不含任何泄露数据集或个人信息。
...
总量仍是 1,但这份证据被**分散**到了它真正可能属于的多个语义结构上。最终 `P(SP)` 与 `P(SF|SFT)` 都在这套软计数上归一。据我们所知,**目前尚无已公开的口令猜测模型这样做**——详见 [与已有工作的关系](#与已有工作的关系--relation-to-prior-work)。
...
SE-PCFG 沿用口令 PCFG 的整体框架,主要区别集中在**训练时如何处理切分歧义**,以及**对中文口令的语义建模**:
...
- 对多条切分做概率加权(软计数)是成熟的 NLP 技术——inside-outside EM、Kudo 2018 的 subword regularization 都这么做;而此前的口令猜测模型一律采用「单切分 + 整数计数」,SE-PCFG 把前者引入了后者的场景。
- 软计数的权重由手设先验、路径罚项与 soft-max 温度一次性算出,不做 EM 迭代,也不用模型自身概率反复重估。
- 角色化姓名形态在定向猜测中已有先例(TarGuess-I 的 N1–N7);这里把它作为一等文法终结符放进撞库(trawling)语法,并加入分隔符吸收(暂不建模复姓)。
- 仓库内并存一份经典的整数单解析实现 [`pcfg_trainer.py`](pcfg_trainer.py)(`defaultdict(int)` / `+= 1`),可作对照基线,在相同猜测预算下比较命中率。
...
# 分析用 · 明文:会在本地落地明文口令(默认已被 .gitignore 排除)
